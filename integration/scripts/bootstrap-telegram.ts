 ...
 * writes its id into `integration/.env`. From then on nothing here needs a person.
 *
 * Run: pnpm --filter @theokit/gateway-integration bootstrap:telegram
 */
...
if (session === undefined) {
  process.stderr.write(
    [
      ...
      "",
      "  pnpm --filter @theokit/gateway-integration session:telegram",
      "",
...
const title = `theokit-integration ${new Date().toISOString().slice(0, 10)}`;
...
await client.sendMessage(chatId, {
  message: "theokit integration fixture — created for live tests",
});
...
process.stdout.write(
  [
    ...
    "",
    "Written to integration/.env. Add the same value as a repository secret for CI.",
    "Run the live suite with: pnpm integration",
    "",
  ].join("\n"),
);
