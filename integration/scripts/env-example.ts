 ...
 *
 * Run: pnpm --filter @theokit/gateway-integration env:example
 */
...
const lines: string[] = [
  ...
  "# Regenerate: pnpm --filter @theokit/gateway-integration env:example",
  "#",
  "# Copy to `integration/.env` and fill in what you have. Missing platforms SKIP with a",
  ...
  "",
  "# Master switch. Without it every live suite skips, so a stray `pnpm integration`",
...
lines.push("#   pnpm --filter @theokit/gateway-integration session:telegram");
...
lines.push("INTEGRATION_PUBLIC_URL=");
lines.push("");
