 ...
 *
 * It writes MATRIX_* into `e2e/.env`, so the suite reads them exactly the way it
 * reads a hosted platform's credentials and cannot tell the difference.
 *
 * Run:
 *   pnpm --filter @theokit/gateway-e2e matrix:up      # boot + provision
 *   pnpm --filter @theokit/gateway-e2e e2e            # run
 *   pnpm --filter @theokit/gateway-e2e matrix:down    # destroy
 */
...
if (bootstrapToken === undefined) {
  process.stderr.write(
    "could not read the bootstrap registration token from the homeserver log.\n" +
      ...
      "run `pnpm --filter @theokit/gateway-e2e matrix:down` and try again.\n",
  );
  process.exit(1);
}

async function register(username: string, token: string) {
  const first = await post("/_matrix/client/v3/register", { username, password: "e2e-local" });
...
const probe = await register("theokit-probe", "theokit-e2e");
process.stdout.write(`registered ${bot.userId} and ${probe.userId}\n`);

const room = await post(
  "/_matrix/client/v3/createRoom",
  { name: "theokit-e2e", preset: "private_chat", invite: [bot.userId] },
  probe.accessToken,
);
...
await put(
  ...
  { msgtype: "m.text", body: "theokit-e2e fixture room" },
  probe.accessToken,
);
...
process.stdout.write(
  [
    ...
    `room:        ${roomId}`,
    "",
    "MATRIX_* written to e2e/.env. Run: pnpm e2e",
    "Tear down with: pnpm --filter @theokit/gateway-e2e matrix:down",
    "",
  ].join("\n"),
);
