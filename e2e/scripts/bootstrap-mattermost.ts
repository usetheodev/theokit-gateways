 ...
 *
 * Run:
 *   pnpm --filter @theokit/gateway-e2e mattermost:up
 *   pnpm --filter @theokit/gateway-e2e e2e
 *   pnpm --filter @theokit/gateway-e2e mattermost:down
 */
...
const team = await api("/teams", {
  body: { name: "theokit-e2e", display_name: "theokit e2e", type: "O" },
  token: session,
});
...
const channel = await api("/channels", {
  body: { team_id: teamId, name: "theokit-e2e", display_name: "theokit e2e", type: "O" },
  token: session,
});
...
const probePat = await api(`/users/${probeId}/tokens`, {
  body: { description: "theokit-e2e-probe" },
  token: session,
});
...
process.stdout.write(
  [
    ...
    `channel:  ${channelId}`,
    "",
    "MATTERMOST_* written to e2e/.env. Run: pnpm e2e",
    "Tear down with: pnpm --filter @theokit/gateway-e2e mattermost:down",
    "",
  ].join("\n"),
);
