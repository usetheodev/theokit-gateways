...
const USER_AGENT = "DiscordBot (https://github.com/usetheokit/theokit-gateways, 0.1) theokit-e2e";
...
describeLiveInbound(DISCORD, "inbound round trip", () => {
  it("receives a message over the gateway, WITH its content", async () => {
    ...
    if (senderToken === undefined) {
      expect
        .soft(senderToken, "add a second bot as DISCORD_TEST_SENDER_TOKEN — see e2e/README.md")
        .toBeUndefined();
      return;
    }
